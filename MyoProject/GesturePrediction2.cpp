	...
	repeat:;
		...
		while (1) {
			...
			if (count == 400) {                                                           // If the current Gesture is counted 200 times
				...
				if (Gesture == "fist") {                                                  // Gesture Performed is Fist
					pass.push_back(1);                                          // Pushing the Id of Gesture performed in the vector pass_performed
					lastGestureRecorded = "fist";
				}
				else if (Gesture == "fingersSpread") {                                    // Gesture Performed is Finger Spread
					pass.push_back(2);                                          // Push
		...
		while (1) {
			...
			if (count == 400) {                                                           // If the current Gesture is counted 200 times
				std::cout << "Gesture Performed : " << Gesture << "\n";
				if (Gesture == "fist") {                                                  // Gesture Performed is Fist
					confirm.push_back(1);                                          // Pushing the Id of Gesture performed in the vector pass_performed
					lastGestureRecorded = "fist";
				}
				else if (Gesture == "fingersSpread") {                                    // Gesture Performed is Finger Spread
					confirm.push_back(2);                                          // Push
					lastGestureRecorded = "fingersSpread";
				}
				else if (Gesture == "waveOut") {
					confirm.push_back(3);
		...
		if (!pose.verify(confirm)) {
			std::cout << "Confirm Password Performed was Wrong !!!!\n"
			std::cout << "Try Again....\n\n"
		}
